    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(this,SIGNAL(toPing()),this, SLOT(ping_state_deal()));
    connect(this,SIGNAL(tologin()),this, SLOT(telnet_login()));
    connect(this,SIGNAL(getMAC()),this, SLOT(getONUMac()));
    connect(this,SIGNAL(toautotest()),this, SLOT(on_pushButton_AutoTest_clicked()));
    connect(this,SIGNAL(tocalibrator82h(int )),this, SLOT(calibrator82h(int )));
    connect(this,SIGNAL(toE5Index()),this, SLOT(getE5Index()));
    connect(this,SIGNAL(toTableIndex()),this, SLOT(readTableIndex()));
    connect(this,SIGNAL(togetTableIndex()),this, SLOT(getTableIndex()));
    ...
    ui->pushButton_start_test->setDisabled(true);
    //ui->pushButton_start_test->setEnabled(true);

    onuType = ui->comboBox_onu_type->currentIndex();
}
...
void MainWindow::on_pushButton_Connect_clicked()
{
    ...
    rts = gpib_connect_initialize(0, 1, &gpib_proc, idn);
    if (rts == CMD_SUCCESS)
        ui->textBrowser->setText("Connection device Success!");
    else{
        ui->textBrowser->setText("Connection device failed!");
        return ;
    }
...
void MainWindow::on_pushButton_reset_clicked()
{
    ...
    if (rts == CMD_SUCCESS)
        ui->textBrowser->append("%Reset Success!");
    else
        ui->textBrowser->append("%Reset Fail!");
...
void MainWindow::on_pushButton_stop_clicked(QString strStartTime)
{
    ...
    qDebug() << "resultjudge" << QString::number(resultjudge,10);
    if(resultjudge == 0){
        out << "\n" << "TEST RESULT: " << "PASS" << "\n";
        out << "End: " << strEndTime << "\r\n";
        ...
        usleep(100000);
        if (excCalibrator == 1){
            ui->pushButton_table->clicked(true);
        }
        else {
            t->sendData("reboot -d 1");
            t->socket()->waitForBytesWritten(3000);
        }
    }else if((resultjudge == 2) || (resultjudge == 3)){
        //调整交叉点
        switch2table(2);
        if (resultjudge == 2)
            crsFlag = 1;
            ...
            crsFlag = 0;
        emit tocalibrator82h(resultjudge);
    }else if((resultjudge == 4) || (resultjudge == 5)){
        //调整消光比
        excCalibrator = 1;
        if (resultjudge == 4)
            excFlag = 1;
            ...
            excFlag = 0;
        emit ui->pushButton_disable_mod->clicked(true);
    }else {
        out << "\n" << "TEST RESULT: " << "FAIL" << "\n";
        out << "End: " << strEndTime << "\r\n";
...
void MainWindow::getBoardSn(QString Sn)
{
    ...
    Sn.replace(5,1,"");
    Sn.replace(2,1,"");
    qDebug() << "getBoardSn" << Sn;
    state = validate_MAC(Sn);
    qDebug() << "state" << state;
...
QString MainWindow::stripCR(const QString &msg)
{
    QString login = "tc login: ";
    QString pwd = "Password:";
    QString pingstate = "192.168.1.1 ping statistics";
    QString pingsuccess = "4 packets transmitted, 4 packets received, 0% packet loss";
    ...
    if(msgGlobal.indexOf(login) >= 0){
        ...
        emit telnetmessage();
    }else if (msgGlobal.indexOf(pingstate) >= 0){
        if (msgGlobal.indexOf(pingsuccess) >= 0)
            emit getMAC();
        else {
            ui->textBrowser->append("ping -c 4 192.168.1.1 error");
            t->socket()->close();
            sleep(1);
            emit tologin();
        }
    }else if ((msgGlobal.indexOf("ifconfig") >= 0) && (msgGlobal.indexOf("HWaddr") >= 0)){
        QString SN  =  msgGlobal.mid(msgGlobal.indexOf("HWaddr")+7,17);
        emit sendSnInfo(SN);
    }else if ((msgGlobal.indexOf("Read") >= 0) && (msgGlobal.indexOf("00000081") >= 0)){
        emit todisablemode();
    }else if ((msgGlobal.indexOf("Read") >= 0) && (msgGlobal.indexOf("00000087") >= 0)){
        emit toorigionmod();
    }else if ((msgGlobal.indexOf("00000080") >= 0) && (msgGlobal.indexOf("000000b4") >= 0)){
        emit calibrate_data();
    }else if ((msgGlobal.indexOf("Read") >= 0) && (msgGlobal.indexOf("000000e5") >= 0)){
        emit toE5Index();
    }else if ((msgGlobal.indexOf("Read") >= 0) && (msgGlobal.indexOf(targetIndex) >= 0)){
        emit togetTableIndex();
    }else if ((msgGlobal.indexOf("Write") >= 0) && (msgGlobal.indexOf("0x0: 0x05") >= 0)){
        emit toTableIndex();
    }else {
...
void MainWindow::telnetConnectionError(QAbstractSocket::SocketError error)
{
    ...
    ui->pushButton_start_test->setEnabled(true);
    ui->textBrowser->append(QString("Connection error: %1").arg(error));
...
{
    qDebug() << "login in";
    ui->textBrowser->append("login in");
...
void MainWindow::telnetLoggedOut()
{
    qDebug() << "login out";
    //ui->pushButton_start_test->setEnabled(true);
    ui->textBrowser->append("login out");
...
int MainWindow::testDataJudge()
{
    ...
    while(i){
        ...
        case EYE_EXTINCTDB:
            ...
            data = data * 2.1092 - 8.4658;
            qDebug() << "EYE_EXTINCTDB" <<data;
            if (onuType == 0){
                if (qAbs(data - 13.0) <= 1.0){
                    ui->label_5_extinctdb->setText("pass");
                    ui->label_5_extinctdb->setFont(QFont("Arial",16,QFont::Bold));
                    pa.setColor(QPalette::WindowText,Qt::green);
                    ui->label_5_extinctdb->setPalette(pa);
                }else {
                    if (data - 14.0 > 0)
                        extstate = 5;
                    else
                        extstate = 4;
                    ui->label_5_extinctdb->setText("fail");
                    ui->label_5_extinctdb->setFont(QFont("Arial",16,QFont::Bold));
                    pa.setColor(QPalette::WindowText,Qt::red);
                    ui->label_5_extinctdb->setPalette(pa);
                }
            }else {
				if (qAbs(data - 11.5) <= 1.0){
                    ui->label_5_extinctdb->setText("pass");
                    ui->label_5_extinctdb->setFont(QFont("Arial",16,QFont::Bold));
                    pa.setColor(QPalette::WindowText,Qt::green);
                    ui->label_5_extinctdb->setPalette(pa);
        ...
        case EYE_PCTCROSS:
            ...
            data = value.toDouble(&ok);
            data = data * 0.5345 + 24.394;
            qDebug() << "PCTCROSS" << data;
            if (qAbs(data - 50.0) < 4.0){
    ...
    if(!rmsjitter.isEmpty()){
        if (!pkpkjitter.isEmpty()){
            ...
            qDebug() << "rmsdata" << rmsdata;
            qDebug() << "pkpkdata" << pkpkdata;
            data1 = rmsdata*14 + pkpkdata;
            data = (data1 * 0.8078) - (14.352e-12);
            qDebug() << "Jitter" << data;
    ...
    qDebug() << "crostate" << QString::number(crostate,10);
...
void MainWindow::write_calibrate_data()
{
    ...
    ui->pushButton_write_flash->clicked(true);
}
...
void MainWindow::translation_data()
{
    ...
    BYTE *p1 = databuf;
    BYTE *p2 = p1+1;
    unsigned short num1change, num1result, num1reserve, num2change, num2result, num2reserve, result,result1;
    bool ok;
    ...
    while (temp_data <= 0xE7){
        ...
        case 0:
            ...
            num2reserve = *p2 & 0x3f;
            num2change = (*p2 >> 6);
            result1 = num1change + num2change;
            qDebug() << "result-1" << QString::number(result1,16);
            result = num1change + num2change - dataAddNum;
...
void MainWindow::on_pushButton_write_flash_clicked()
{
    ...
    t->sendData("/userfs/bin/readDataFromBob");
    t->socket()->waitForBytesWritten(3000);
...
void MainWindow::on_pushButton_3_set_mod_clicked()
{
    ...
    if (excFlag == 1){
        data_87h = data_87h + reduceNum_db;
    }
    else
        data_87h = data_87h - reduceNum_db;
    qDebug() << "set_mod_clicked -> data_87h" << QString::number(data_87h,16);

    QString cmd_A2_wr = QString("sifm xw 0 0xc7 0xd1 1 0x87 1 0x%1").arg(QString::number(data_87h,16));
    t->sendData(cmd_A2_wr);
    t->socket()->waitForBytesWritten(3000);
    
    ui->lineEdit_2_read_origion_mod->setText(QString::number(data_87h,16));
    emit toautotest();
}
...
void MainWindow::disable_mod_set()
{
    ...
    emit ui->pushButton_3_set_mod->clicked(true);
}
...
void MainWindow::ping_state_deal()
{
    //t->sendData("dmesg -c");
    //t->socket()->waitForBytesWritten(3000);
    t->sendData("ping -c 4 192.168.1.1");
    t->socket()->waitForBytesWritten(3000);
}

void MainWindow::getONUMac()
{
    //t->sendData("dmesg -c");
    //t->socket()->waitForBytesWritten(3000);
    t->sendData("ifconfig");
...
void MainWindow::calibrator82h(int errorState)
{   
    qDebug() << "calibrator82h -> data_82h" << QString::number(data_82h,16);
    qDebug() << "calibrator82h -> errorState" << errorState;
    QPalette pa;

    if (crsFlag == 1){
        data_82h = data_82h - reduceNum;
    }
    else
        data_82h = data_82h + reduceNum;
    ...
    if (errorState == 2){
        if (data_82h <= 0xc0){
            ui->textBrowser->append("82h register calibrator done");
            ui->textBrowser->append("82h register calibrator error");
            ui->pushButton_start_test->setEnabled(true);
            ui->label_2_judge_state->setText("fail");
            ui->label_2_judge_state->setFont(QFont("Arial",36,QFont::Bold));
            pa.setColor(QPalette::WindowText,Qt::red);
    ...
    QString cmd_A2_wr = QString("sifm xw 0 0xc7 0xd1 1 0x82 1 %1").arg(QString::number(data_82h,16));
    t->sendData(cmd_A2_wr);
    t->socket()->waitForBytesWritten(3000);
    t->sendData("/userfs/bin/readDataFromBob");
    t->socket()->waitForBytesWritten(3000);
    t->sendData(QString("/userfs/bin/rw_table write /tmp/etc/MTK_A2.ini 642 1 %1").arg(QString::number(data_82h,16)));
    t->socket()->waitForBytesWritten(3000);
    t->sendData("/userfs/bin/savedataTobob");
    t->socket()->waitForBytesWritten(3000);
    t->sendData("/userfs/bin/mtd -f write /tmp/etc/BOB.ini BOB");
...
{
    bool ok;
    QString str_index = ui->pushButton_table->text();
    BYTE  index = str_index.toInt(&ok,16);

    //switch2table(index);
    t->sendData("sifm xr 0 0xc7 0xd1 1 0xe5 1");
    t->socket()->waitForBytesWritten(3000);
    t->sendData("dmesg -c");
    t->socket()->waitForBytesWritten(3000);
}

void MainWindow::getE5Index()
{
    E5Index= msgGlobal.mid(msgGlobal.indexOf("000000e5") + 12, 2);

    switch2table(5);
}

void MainWindow::readTableIndex()
{
    bool ok;
    char num = E5Index.toInt(&ok,16);
    char index = num - 65;
    char index1 = index >> 1;
    char addr0 = index1 + (index1 >> 2) + 0x80;
    QString addr0Str = QString::number(addr0,16);
    targetIndex = addr0Str.mid(addr0Str.indexOf("ffffffffffffff") + 14, 2);

    t->sendData(QString("sifm xr 0 0xc7 0xd1 1 %1 2").arg(targetIndex));
    t->socket()->waitForBytesWritten(3000);
    t->sendData("dmesg -c");
    t->socket()->waitForBytesWritten(3000);
}

void MainWindow::getTableIndex()
{
    QString data1Index= msgGlobal.mid(msgGlobal.indexOf(targetIndex) + 6, 2);
    QString data2Index= msgGlobal.mid(msgGlobal.indexOf(targetIndex) + 11, 2);
    qDebug() << "data1Index" << data1Index;
    qDebug() << "data2Index" << data2Index;

    bool ok;
    QString target= data1Index + data2Index;
    qDebug() << "target" << target;
    int indexNum = target.toInt(&ok,16);
    qDebug() << "indexNum" << QString::number(indexNum,10);

    unsigned short index = E5Index.toInt(&ok,16);
    unsigned short index1 = index - 0x41;
    int sum = (index1 >> 1) & 0x03;
    switch(sum){
    case 0:
        E5TargetIndex = indexNum >> 6;
        qDebug() << QString::number(E5TargetIndex,10);
        break;
    case 1:
        E5TargetIndex = (indexNum >> 4) & 0x3ff;
        qDebug() << QString::number(E5TargetIndex,10);
        break;
    case 2:
        E5TargetIndex = (indexNum >> 2) & 0x3ff;
        qDebug() << QString::number(E5TargetIndex,10);
        break;
    case 3:
        E5TargetIndex = indexNum & 0x3ff;
        qDebug() << QString::number(E5TargetIndex,10);
        break;
    default:
        break;
    ...
    QString index87H = ui->lineEdit_2_read_origion_mod->text();
    unsigned short index87HNum = index87H.toInt(&ok,16);
    qDebug()<< "index87HNum" << index87HNum;
    qDebug()<< "E5TargetIndex" << E5TargetIndex;
    diff = E5TargetIndex - index87HNum * 4;
    ui->lineEdit_diffrence->setText(QString::number(diff,16));

    emit toreaddata();
}


void MainWindow::on_comboBox_onu_type_currentIndexChanged(int index)
{
    onuType = index;
}
